...
連絡してもらう形。自動では一覧に出ないので、精度の低い問い合わせを弾ける。

## 問い合わせの受け口を変える

`index.html` の `CONTACT` を書き換えるだけで、ページ全体の文言とボタンが切り替わる。

```js
var CONTACT = {
  kind: "threads",
  value: "nailsalon_support"
};
```

| kind | value に入れるもの | 相手の操作 |
|---|---|---|
| `email` | メールアドレス | ボタン → **件名・本文入りのメールが開く → 送信を押すだけ**（コピー不要） |
| `line` | 友だち追加URL | ボタン → コピーされてLINEが開く → 貼って送る |
| `instagram` | @なしのID | ボタン → コピーされてDMが開く → 貼って送る |
| `threads` | @なしのID | ボタン → コピーされてThreadsが開く → 貼って送る |
| `form` | フォームのURL | ページ内フォームの代わりに「フォームを開く」ボタン1つになる |

`email` だけはコピーの手順が要らないので、いちばん手数が少ない。
